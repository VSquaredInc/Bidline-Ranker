...
**0700z landing-tail columns (v1.15.0, 2026-09-18):** A date column whose only content is an arrival (airport + time, no flight number, nothing after it) before 0700 UTC is the tail of the *previous* day's flight spilling past midnight — not a new workday. `colWorkingStatus` in `parseBidlineWithPositions()` now marks these columns off (`isEarlyArrivalOnly`), so they no longer extend `trips[]`. Root cause: the 0700z detector already existed (`earlyArrivalDates`, v1.14.0) but was wired only into the vacation-slide "last day" checkbox path, not into trip-boundary computation itself — so a trip's real last workday (e.g. Oct 31, landing ORD 1:20 the next calendar day) was still being read as ending on the calendar date the landing printed on (Nov 1), making an unrelated vacation starting that date look like it conflicted with the trip by one day when it didn't touch it at all. **Critical distinction:** a column with a flight number OR a second time token is real duty (e.g. a same-UTC-day international redeye departing 0:30 arriving 6:10) and must NOT be caught by this rule — verified against real data on line 4141 (Oct 2026 ORD 747 FO): Oct 5 (VN311, real flight) and Oct 27 (CI107 + legs) stayed workdays; only Nov 1 (bare `ORD 1:20`) was reclassified. This ripples fleet-wide — confirmed via a full FO/CA diff against the pre-fix parser (33/128 ORD 747 FO lines, 13/50 ORD 747 CA lines had `trips[]` change) — front/back DH counts and X-marker `datesOff` were unaffected (0 changes on both) since neither is driven by this column.
