...
**Per-candidate 7.D.2.d abut (v1.9.1, fixed 2026-07-15, same day):** Making `locDaysOff` uncapped exposed a real exploit: `isPartiallyConflicted` (full vs. partial classification) was computed once from the *original* awarded vacation and used to gate the abut-rule check across the *entire* candidate search. A fully-conflicted original vacation could therefore slide to a position that only barely grazes the last day of a wholly unrelated, earlier trip pairing — a "candidate" position that is itself partially conflicted (real workdays mixed with already-off days) — and harvest that unrelated trip's entire uncapped 25.K.3.a leading edge as free days off, without ever being required to abut a Day Off. Confirmed with a synthetic 3-trip schedule: the search preferred grazing an unrelated trip's tail (`effectiveDaysOff` 20, `locDaysOff` 6 from a trip barely touched) over properly resolving the actually-conflicting trip (`effectiveDaysOff` 18). Fixed by evaluating the 7.D.2.d abut requirement **per candidate** (`r.vacWorkdays > 0 && r.vacWorkdays < vacLength` at that specific position), not from the origin's classification — the 7.D.2.c cap search stays origin-based (that rule is specifically about how far a partially-conflicted *award* may slide). Regression test `abut-1` through `abut-5` in `tools/test-vacation-slide.js` pins this down.
