...
**7.D.2.c cap must also stop on losing contact with the conflicting trip, not just on reaching full conflict (v1.15.2, 2026-09-18).** A second, distinct leapfrog vector from the v1.9.1 one above: `effectiveMinStart`/`effectiveMaxStart` only advanced past a candidate position when `cand.vacWorkdays >= vacLength` (full conflict). That condition can never fire when the conflicting trip is SHORTER than the vacation itself — no window of vacation-length can ever be 100% inside a shorter trip — so the search silently fell through to the un-capped anchor bound (`vacStart`/`vacEnd`) and could wander onto a completely different, later trip. Reported case: line 4185 (Oct 2026 ORD 747 FO) — a 5-day trip Oct 28–Nov 1 touched by only its last day by a 7-day vacation awarded Nov 1–7; old code recommended sliding to **Nov 7–13**, landing on an unrelated Nov 11–15 trip instead of resolving the Oct 28–Nov 1 conflict. Fix: the boundary search now also stops the moment a candidate no longer overlaps ANY trip the *original* award conflicted with, independent of the full-conflict check — whichever stop condition is reached first. Correct result: slide **Oct 31–Nov 6** (LOC 3, absorbing Oct 28–30 leading edge). Regression tests `nlf1`–`nlf3` in `tools/test-vacation-slide.js` pin this case.

**Capping the starting position isn't enough — the window's far END can still reach an unrelated trip (v1.15.2, same day).** Follow-up the domain owner caught immediately: even with the above fix, a candidate whose *sStart* stays within the (now-correctly-capped) range can have its *sEnd* land on a completely different trip. Example: a trip ends Nov 2 (touching only the vacation's first day), a wholly unrelated trip starts Nov 8 (the day right after a Nov 1–7 vacation ends) — sliding the start to Nov 2 is still "in range" per the sStart cap, but the resulting window (Nov 2–8) reaches all the way to the unrelated trip's first day and would credit its vacWorkdays/LOC. Fixed with a second, independent guard in the main position-generation loop: when `isPartiallyConflicted`, any candidate whose window `[sStart, sEnd]` overlaps a trip **not** in `origConflictTrips` is rejected outright — separate from and in addition to the start-position cap. Verified this doesn't touch the *origin* (unslid) position's own legitimate 7.D.7/31.B/25.K.3.a edge-adjacency credit — those provisions describe whatever naturally borders the awarded position, slid or not, and aren't restricted to "the conflicting trip"; only a *window's own span* (actual vacation days, i.e. `vacWorkdays`) is restricted to the trip in conflict. Regression tests `wr1`–`wr3` in `tools/test-vacation-slide.js` pin this case.
...
**0700z landing-tail columns (v1.15.0-1.15.1, 2026-09-18):** A date column whose only content is an arrival (airport + time, no flight/aircraft designator, no second time) before 0700 UTC is the tail of the *previous* day's flight spilling past midnight — not a new workday. `colWorkingStatus` in `parseBidlineWithPositions()` now marks these columns off (`isEarlyArrivalOnly`), so they no longer extend `trips[]`. Root cause: the 0700z detector already existed (`earlyArrivalDates`, v1.14.0) but was wired only into the vacation-slide "last day" checkbox path, not into trip-boundary computation itself — so a trip's real last workday (e.g. Oct 31, landing ORD 1:20 the next calendar day) was still being read as ending on the calendar date the landing printed on (Nov 1), making an unrelated vacation starting that date look like it conflicted with the trip by one day when it didn't touch it at all. **Critical distinction:** any digit-bearing token that isn't a time or a pure 3-letter airport code (flight number, aircraft type, etc.) means a departure was scheduled from this date — real duty, must NOT be caught by this rule. **v1.15.0 shipped this matched on length (`/^\w{4,}/`, inherited from the pre-existing `hasContent` flight-number check) and was wrong** — line 4204's Nov 6 flight "588" is only 3 characters and would have been misread as a bare landing, zeroing out a real workday; v1.15.1 fixed it to match on "contains a digit" instead, no length minimum. Verified against real data on line 4141 (Oct 2026 ORD 747 FO): Oct 5 (VN311, real flight) and Oct 27 (CI107 + legs) stayed workdays; only Nov 1 (bare `ORD 1:20`) was reclassified. This ripples fleet-wide — confirmed via a full-fleet audit dumping every flagged column across all 10 base/aircraft combos in `Current/` (ANC/JFK/LAX/MIA/ORD × CA/FO): every one matched the genuine-tail signature (airports + `*` deadhead markers + exactly one time, zero digit tokens), none were false positives. A pre-fix-vs-post-fix parser diff showed 33/128 ORD 747 FO lines and 13/50 ORD 747 CA lines had `trips[]` change; front/back DH counts and X-marker `datesOff` were unaffected (0 changes on both) since neither is driven by this column.
